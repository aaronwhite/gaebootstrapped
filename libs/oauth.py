...
class LinkedInClient(OAuthClient):
  ...
  def _lookup_user_info(self, access_token, access_secret):
    ...
    response = self.make_request("http://api.linkedin.com/v1/people/~:"
                                 "(picture-url,headline,id,first-name,last-name,public-profile-url)",
                                 token=access_token,
    ...
    try:
      ...
      user_info['picture'] = 'http://www.gravatar.com/avatar/00000000000000000000000000000000?d=mm&s=50'
    user_info["name"] = data["firstName"] + " " + data["lastName"]
    user_info["headline"] = data.get('headline')
    user_info["profile_url"] = data.get('publicProfileUrl')
